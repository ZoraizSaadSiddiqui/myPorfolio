...
const Platform = () => {
  return (
    <section className="mt-10" id="tools">
      <h2 className="text-center text-pink-600 text-[50px] font-bold mt-9 mb-12">
        Platforms & Tools
      </h2>
<div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-1 justify-items-center px-4 sm:px-8 text-center mb-12">
  {tools.map((tool, i) => (
    ...
    >
      <div className="text-pink-600 text-6xl mb-5">{tool.icon}</div>
      <span className="text-[17px] font-semibold text-pink-700">
        {tool.label}
      </span>
    </motion.div>
  ))}
